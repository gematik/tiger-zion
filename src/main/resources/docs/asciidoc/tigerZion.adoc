...
In this paragraph we'll take a look at the various structures that can be serialized with the RbelWriter, which sits at the core of the Zion-Server. Therefore you need to activate the RbelWriter in your tiger.yaml.
...
For this the key at `$.signature.verifiedUsing` is consulted and a matching key is searched in the tiger key-database. Therefore you need to add the keyfolders in your tiger.yaml.
