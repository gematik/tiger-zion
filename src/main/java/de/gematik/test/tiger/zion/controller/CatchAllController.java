...
public class CatchAllController implements WebMvcConfigurer {
  ...
  public ResponseEntity<byte[]> masterResponder(
      RequestEntity<byte[]> request, HttpServletRequest servletRequest) throws UnknownHostException {
    ...
    final RbelSocketAddress client =
        RbelSocketAddress.builder()
            .address(RbelInternetAddress.fromInetAddress(InetAddress.getByName(servletRequest.getRemoteHost())))
            .port(servletRequest.getRemotePort())
            .build();
    final RbelSocketAddress server =
        RbelSocketAddress.builder()
            .address(RbelInternetAddress.fromInetAddress(InetAddress.getByName(servletRequest.getLocalAddr())))
